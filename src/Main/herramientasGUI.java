...
public class herramientasGUI {
    ...
    public BufferedImage LeerImagen(){
        ...
        FileNameExtensionFilter filter = new FileNameExtensionFilter("Imagenes","png","jpeg","bmp","jpg");
        fc.addChoosableFileFilter(filter);
        fc.setAcceptAllFileFilterUsed(false);
        if (fc.showOpenDialog(null) == 0) {
            try {
                BufferedImage imagen = ImageIO.read(fc.getSelectedFile());
                //imagen = (BufferedImage)imagen.getScaledInstance(620, 400, Image.SCALE_DEFAULT);
                this.setImagenR(imagen.getScaledInstance(191, 138, Image.SCALE_DEFAULT));
                return imagen;
    ...
    public ArrayList<String[]> verificaCSV(ArrayList<String[]> valores, baseDeDatos bd) throws SQLException{
        ...
        for(int i=0;i<valores.get(0).length;i++){
            if(valores.get(0)[i].compareToIgnoreCase("Zona")==0){
                indices[9]=i;
                i=valores.get(0).length;
            }
        }
        for(int i=0;i<valores.size();i++){
            if(valores.get(i)[indices[1]].compareToIgnoreCase("")!=0){
                String[] aux = new String[valores.get(i).length];
                ...
                if(valores.get(i)[indices[6]].compareTo("")==0){
                    aux[6]="0";
                }else{
                    aux[6]=valores.get(i)[indices[6]];
                }
                if(valores.get(i)[indices[7]].compareTo("")==0){
                    aux[7]="0";
                }else{
                    aux[7]=valores.get(i)[indices[7]];
                }
                if(valores.get(i)[indices[8]].compareTo("")==0){
                    aux[8]="0";
                }else{
                    aux[8]=valores.get(i)[indices[8]];
                }
                aux[9]=valores.get(i)[indices[9]];
